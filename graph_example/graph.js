...
var Node = Class.extend({
    init: function(id, a)
    {
        ...
        if (a.text != false)
        {
            ...
            div.style.fontWeight = (a.fontweight) ? a.fontweight : "";
    ...
    flatten: function(depth, traversable, _visited) {
        ...
        if (traversable === undefined) traversable = function(node, edge)
        {
            return true;
        };
        _visited = _visited || {};
...
var Graph = Class.extend(
{
    ...
    drag: function(mouse)
    {
        ...
        if (mouse.pressed)
        {
            pt = mouse.relative && mouse.relative(this.canvas) ||
            {
                x: mouse.x,
                y: mouse.y
            };
...
GraphSpringLayout = GraphLayout.extend({
    ...
    {
        var a = this._distance(node1, node2);
        dx = a[0];
        dy = a[1];
        d = a[2];
        d2 = a[3];
...
Graph.depthFirstSearch = function(node, a)
{
    if (a === undefined)
        a = {};
    if (a.visit === undefined) a.visit = function(node)
    {
        return false;
    };
    if (a.traversable === undefined) a.traversable = function(node, edge)
    {
        return true;
    };
    var stop = visit(node);
    ...
    for (var i = 0; i < node.links.length; i++)
    {
        var n = node.links[i];
        if (stop) return true;
        if (a.traversable(node, node.links.edge(n) == false))
            continue;
        if (!a._visited[n.id])
        {
...
Graph.breadthFirstSearch = function(node, a)
{
    ...
    if (a.traversable === undefined)
        a.traversable = function(node, edge)
        {
            return true;
        };
    var q = [node];
...
var Heap = Class.extend(
{
    ...
    {
        var i = 0;
        while (i <= this.w.length && weight < (this.w[i] || Infinity)) i++;
...
Graph.brandesBetweennessCentrality = function(graph, a)
{
    ...
    var W = Graph.adjacency(graph, a);
    var b = {};
    for (var n in graph.nodeset) b[n] = 0.0;
    for (var id in graph.nodeset)
    {
        ...
        var S = [];
        var E = {};
        for (var n in graph.nodeset) E[n] = 0;
        E[id] = 1.0;
        while (Q.length)
        {
            var q = Q.pop();
        ...
        var d = {};
        for (var v in graph.nodeset) d[v] = 0;
        while (S.length)
        {
            var w = S.pop();
...
Graph.eigenvectorCentrality = function(graph, a)
{
    ...
    var G = Graph.adjacency(graph, a);
    var v = {};
    for (var n in graph.nodeset) v[n] = Math.random();
    normalize(v);
    for (var i = 0; i < a.iterations; i++)
    {
        var v0 = v
        var v = {};
        for (var k in v0) v[k] = 0;
        for (var n1 in v)
        {
            for (var n2 in G[n1])
            {
        ...
        normalize(v);
        var e = 0;
        for (var n in v) e += Math.abs(v[n] - v0[n]);
    ...
    var x = {};
    for (var n in graph.nodeset) x[n] = 0;
    return x;
};
...
Graph.partition = function(graph)
{
    ...
    for (var i = 0; i < graph.nodes.length; i++)
    {
        ...
        n = n.flatten();
        var d = {};
        for (var j = 0; j < n.length; j++) d[n[j].id] = true;
    ...
    {
        var n = [];
        for (var j = 0; j < g[i].length; j++) n[j] = graph.nodeset[g[i][j]];
