...
function len(array) {
    ...
    (function() {
        ...
        var has_super = /xyz/.test(function() {
            xyz;
        }) ? /\b_super\b/ : /.*/;
        this.Class = function() {};
        Class.extend = function(properties) {
            ...
            for (var k in properties) {
                p[k] = typeof properties[k] == "function" && typeof _super[k] == "function" && has_super.test(properties[k]) ? (function(k, f) {
                    return function() {
                        var s, r;
                        s = this._super;
                        this._super = _super[k];
...
var Geometry = Class.extend({
    ...
    Bounds: Class.extend({
        ...
        intersects: function(b) {
            return Math.max(this.x, b.x) < Math.min(this.x + this.width, b.x + b.width) && Math.max(this.y, b.y) < Math.min(this.y + this.height, b.y + b.height);
        },
        ...
        contains: function(pt) {
            ...
            if (pt instanceof Bounds) {
                return pt.x >= this.x && pt.x + pt.width <= this.x + this.width && pt.y >= this.y && pt.y + pt.height <= this.y + this.height
            }
        }
    })
});
...
var Color = Class.extend({
    ...
    rotate: function(angle) {
        ...
        return new Color(hsb[0], hsb[1], hsb[2], this.a, {
            "colorspace": HSB
        });
    },
    adjust: function(options) {
        ...
        return new Color(hsb[0] % 1, hsb[1], hsb[2], this.a, {
            "colorspace": HSB
        });
    }
});
...
function _rgb2hex(r, g, b) {
    ...
    return "#" + parseHex(Math.round(r * 255)) + parseHex(Math.round(g * 255)) + parseHex(Math.round(b * 255));
}
...
function analog(clr, angle, d) {
    ...
    return new Color(hsb[0], hsb[1], hsb[2], clr.a, {
        "colorspace": HSB
    });
}
...
function _colorMixin(options) {
    ...
    if (options === undefined) {
        ...
        return [
            ...
            o.fill : new Color(o.fill) : s.fill, (o.stroke !== undefined) ?
            (o.stroke instanceof Color) ?
            o.stroke : new Color(o.stroke) : s.stroke, (o.strokewidth !== undefined) ? o.strokewidth :
            (o.strokeWidth !== undefined) ? o.strokeWidth : s.strokewidth, (o.strokestyle !== undefined) ? o.strokestyle :
            (o.strokeStyle !== undefined) ? o.strokeStyle : s.strokestyle, (o.linecap !== undefined) ? o.linecap :
...
var AffineTransform = Transform = Class.extend({
    ...
    invert: function() {
        ...
        this.matrix = [
            m[4] / d, -m[1] / d, 0, -m[3] / d,
            m[0] / d, 0, (m[3] * m[7] - m[4] * m[6]) / d, -(m[0] * m[7] - m[1] * m[6]) / d, 1
        ];
    },
    inverse: function() {
        var m = this.copy();
...
var Bezier = Class.extend({
    ...
    segmentLengths: function(path, relative, n) {
        ...
        if (relative == true) {
            ...
            if (s > 0) {
                return Array.map(lengths, function(v) {
                    return v / s;
                });
            } else {
                return Array.map(lengths, function(v) {
    ...
    point: function(path, t, segments) {
        var _, i, closeto;
        _ = this._locate(path, t, segments);
        i = _[0];
        t = _[1];
        closeto = _[2];
    ...
    findPath: function(points, curvature) {
        ...
        dx[points.length - 1] = 0;
        var dy = {
            0: 0
        };
        dy[points.length - 1] = 0;
        var bi = {
            1: 1 / curvature
        };
        var ax = {
            1: (points[2].x - points[0].x - dx[0]) * bi[1]
        };
        var ay = {
            1: (points[2].y - points[0].y - dy[0]) * bi[1]
        };
...
var Path = BezierPath = Class.extend({
    init: function(path) {
        if (path === undefined) {
            ...
            this.array = Array.map(path.array, function(pt) {
                return pt.copy();
            });
        } else if (path instanceof Array) {
            this.array = Array.map(path, function(pt) {
                return pt.copy();
            });
        }
        this._clip = false;
...
function _fontMixin(options) {
    ...
    if (options === undefined) {
        ...
        return [
            (o.font) ? o.font : s.fontname, (o.fontsize !== undefined) ? o.fontsize :
            (o.fontSize !== undefined) ? o.fontSize : s.fontsize, (o.fontweight !== undefined) ? o.fontweight :
            (o.fontWeight !== undefined) ? o.fontWeight : s.fontweight, (o.lineheight !== undefined) ? o.lineheight :
...
function noise(x, y, m) {
    ...
    return (Math.clamp(n * m, -1, +1) + 1) * 0.5;
}
...
window._requestFrame = function(callback, canvas, fps) {
    var f = window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame || window.msRequestAnimationFrame || window.oRequestAnimationFrame || function(callback, element) {
        return window.setTimeout(callback, 1000 / (fps || 60));
    };
    return f(Function.closure(canvas, callback), canvas.element);
};
window._clearFrame = function(id) {
    var f = window.cancelAnimationFrame || window.webkitCancelAnimationFrame || window.webkitCancelRequestAnimationFrame || window.mozCancelAnimationFrame || window.mozCancelRequestAnimationFrame || window.msCancelAnimationFrame || window.msCancelRequestAnimationFrame || window.oCancelAnimationFrame || window.oCancelRequestAnimationFrame || window.clearTimeout;
    return f(id);
};
_ctx = null;
...
var Canvas = Class.extend({
    ...
    run: function() {
        ...
        var _preload = function() {
            if (cache.busy > 0) {
                setTimeout(Function.closure(this, _preload), 10);
                return;
            }
            this._draw();
        }
...
var OffscreenBuffer = Buffer = Canvas.extend({
    init: function(width, height) {
        ...
        this._super(document.createElement("canvas"), width, height, {
            mouse: false
        });
...
function colorize(img, color, bias) {
    ...
    var m2 = new Color(bias || [0, 0, 0, 0]).map({
        base: 255
    });
    return filter(img, function(p) {
        return [
        ...
        case HARDLIGHT:
            op = function(x, y, luminance) {
                ...
                return (luminance < 0.45) ? a :
                    (luminance > 0.55) ? b :
                    geometry.lerp(a, b, (luminance - 0.45) * 255);
            };
            break;
        default:
            op = function(x, y) {
...
function widget(canvas, variable, type, options) {
    ...
    if (canvas.variables[v] === undefined) {
        ...
        if (type == STRING || type == TEXT) {
            ...
            var f = function(e, p) {
                canvas.variables[this.id] = this.value;
                cb(e, p);
            };
        } else if (type == NUMBER) {
            var s = "<input type='text' id='" + v + "' value='" + (o.value || 0) + "'/>";
            var f = function(e, p) {
                canvas.variables[this.id] = parseFloat(this.value);
                cb(e, p);
            };
        } else if (type == BOOLEAN) {
            var s = "<input type='checkbox' id='" + v + "'" + ((o.value == true) ? " checked" : "") + "/>";
            var f = function(e, p) {
                canvas.variables[this.id] = this.checked;
                cb(e, p);
            };
        } else if (type == RANGE) {
            var s = "<input type='range' id='" + v + "' value='" + (o.value || 0) + "'" + " min='" + (o.min || 0) + "' max='" + (o.max || 1) + "' step='" + (o.step || 0.01) + "'/>";
            var f = function(e, p) {
                canvas.variables[this.id] = parseFloat(this.value);
                cb(e, p);
            };
            ...
            f = function(e, p) {
                canvas.variables[this.id] = this.options[this.selectedIndex].value;
                cb(e, p);
            };
        ...
        if (type != FUNCTION) {
            attachEvent(e.lastChild, "change", f);
            e.lastChild.change(null, false);
        } else {
...
function _stackblur(img, buffer, radius) {
    ...
    function stackBlurCanvasRGBA(a, b, c, d, e, f) {
        ...
        var i;
        i = h.getImageData(b, c, d, e);
        var k = i.data;
        var l, m, n, o, p, q, r, s, t, u, v, w, x, y, z, A, B, C, D, E, F, G, H, I;
        var J = f + f + 1;
        var K = d << 2;
        var L = d - 1;
        var M = e - 1;
        ...
        var T = null;
        r = q = 0;
        var U = _stackblur_mul[f];
        var V = _stackblur_shg[f];
        for (m = 0; m < e; m++) {
            A = B = C = D = s = t = u = v = 0;
            w = N * (E = k[q]);
            x = N * (F = k[q + 1]);
    ...
    function stackBlurImage(a, b, c, d) {
        ...
        if (d) stackBlurCanvasRGBA(b, 0, 0, f, g, c);
        else stackBlurCanvasRGB(b, 0, 0, f, g, c)
    }
    ...
    document.createElement("canvas").getContext || (function() {
        ...
        var M = {
            ...
            initElement: function(b) {
                if (!b.getContext) {
                    ...
                    var a = b.attributes;
                    if (a.width && a.width.specified) b.style.width = a.width.nodeValue.replace("px", "") + "px";
                    else b.width = b.clientWidth;
                    if (a.height && a.height.specified) b.style.height = a.height.nodeValue.replace("px", "") + "px";
        ...
        function Z(b) {
            ...
            switch (b.propertyName) {
                case "width":
                    a.style.width = a.attributes.width.nodeValue.replace("px", "") + "px";
                    a.getContext().clearRect();
                    break;
                case "height":
                    a.style.height = a.attributes.height.nodeValue.replace("px", "") + "px";
                    a.getContext().clearRect();
                    break
            }
        }
